...
conversion_trait(::Type{<:LineHexbin}) = PointBased()

function data_limits(hb::LineHexbin)
...
boundingbox(p::LineHexbin, space::Symbol = :data) = apply_transform_and_model(p, data_limits(p))

get_weight(weights, i) = Float64(weights[i])
get_weight(::StatsBase.UnitWeights, i) = 1e0
get_weight(::Nothing, i) = 1e0

function plot!(hb::LineHexbin{<:Tuple{<:AbstractVector{<:AbstractVector{<:Point2}}}})
    xy = hb[1]
