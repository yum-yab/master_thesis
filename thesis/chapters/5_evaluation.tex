...
The first simple evaluation is to look at the change of share of variability encoded by each \ac{eof} (see Equation~\ref{eq:eof variance calculation}). 
...
The same analysis with the \ac{ivt} patterns (Figure~\ref{fig:ivt mode variability}) reveal a general upwards trend in the primary mode of \ac{ivt}, from median $26\%$ in the first window to around $28\%$ in the last. 
...
\todo{Maybe I should just cut out the third pattern since I do not use it anywhere else, and it seems degenerate, but also this is why I assume it is degenerate and not worth of analyzing further}
...
Similar to the other Figures in this Section, Figure~\ref{fig:pr eof evolution} shows the evolution of dominant precipitation \ac{eof} patterns. 
...
\todo{Maybe add boxplot of total variance (sum of all singular values squared) to investigate sources for \ac{ivt} SD rise}
...
The relationships between mode 1 and mode 2 of \ac{ivt} and PSL and vice versa are pretty similar (Figure in the Appendix\todo{Figure in the Appendix}): 
...
From Figure~\ref{fig:cor pr ivt modes11} it is obvious that the primary modes of \ac{ivt} and precipitation are very closely related. 
...
Since the correlation between \ac{ivt} and \ac{nao}/\ac{eap} and precipitation and \ac{ivt} are already known, the analysis of their temporal correlation yields no surprises. 
...
\todo{Nur mathematische/stat analyse -> verbindung mit physikalischen Realität schwierig und aufgabe von domain experten}
...
Since the \ac{eof} coefficients have a SD of one in their normal (unscaled) state, it is entirely dependent on the scaling, encoded in the singular values of the respective mode. \todo{THink about this. is this true?} 
...
\todo{Maybe leave that out, too bold statement}
