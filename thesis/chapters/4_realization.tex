...
In the following the EOF procedure for one such scope is described, but is executed the same for each scope. 
...
For this the SVD implementation of Julia's \texttt{LinearAlgebra} package (which is part of the Standard Library) is used.
...
The values $S$ can be used to compute the share of variablity encoded in that corresponding mode $i$ with 
...
In contrast to \cite{vietinghoffdiss}, the scaling is done at the loading of the data to be more flexible (see Section~\ref{sec:mode_scaling}).
...
As explained in Section~\ref{sec:eof}, the resulting patterns are eigenvectors, meaning that scaled with any arbitrary scalar $\gamma$ is also a valid result. 
...
The only work using such a large scale analysis is by \cite{vietinghoffdiss} \cite{vietinghoffdiss}.  
...
This process is illustrated in the Figure TODO \todo{Make fig with the mean fieds, cleaned by spatial mean and the first EOF pattern} 
This has the effect that positive values of patterns align with above-average values of the actual data mean and vice versa, making the sign of the EOFs interpretable (see Figure TODO). 
...
If the absolute correlation boxplot shows a clear trend (e.g. consistent values) but the plot with normal correlation values has either no visisble correlation (meaning the alignment is very arbitrary an alignment varies a lot) or there are many outliers on the opposite side of the zero line (alignment did not work for just some members). 

To fix these problems and also measure correlation 
