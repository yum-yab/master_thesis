...
Hexbin plots are very similar, except they divide the grid into hexagonal \enquote{bins} and handle the oberservations in the same way as heatmaps. 
...
Unfortunately, it is very hard to visualize them on ensemble scale, since each member is quite diffenet in each month, resulting in a hard to interpret mess when using mean functions or boxplots. 

So instead the temporal activity can be used to evaluate the relationship of certain patterns with other variables or even other patterns. 
Of course, it is very important that the same members are compared to each other. 
