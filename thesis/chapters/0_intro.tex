...
Especially the last decades the climate crisis gained more and more attention, leading to the creation of multiple international organizations and institutions (e.g. the \ac{ipcc} in 1988). \todo{Weiß nciht ob so sinnvoll hier bei adam und eva anzufangen...}
...
These oscillation systems, which play a crucial role in regulating weather patterns.
%, are experiencing shifts that have yet to be fully understood or explored. 
...
Furthermore, this pattern analysis approach addresses one of the major challenges in modern climate science: the uncertainty associated with multiple simulation results. 
...
%   \item Give an example with the \ac{nao}
% \end{itemize}
...
% The difference per year is called the \ac{nao} index and can be seen in Figure \ref{fig:naoindex_comparison}. 
...
It is defined as the second Principal Component (\ac{eof}) of Sea Level Pressure in the Atlantic.  
...
% THos include the patterns of the quantified moisture transport, preciptation and surface level pressure (PSL), which can be used as the \ac{nao} index (see Figure~\ref{fig:naoindex_comparison}) and can be used t
...
The first connection should be to the \ac{nao}, or generally, patterns of surface pressure levels (like the East Atlantic Pattern, the second most significant mode of PSL \ac{eof}). 
...
One interesting thing to find if \ac{ivt} (or other variables) experience a similar shift to the north similar to the results of \citeauthorwork{vietinghoff_visual_2021}.
...
%   \item[\textbf{M3}] How to implement a sliding window \ac{eof} pattern calculation similar to \citeauthorwork{vietinghoffdiss}?
