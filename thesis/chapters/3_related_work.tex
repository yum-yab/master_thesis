...
Published in 1982, \citeauthor{salstein_modes_1983} provided the first example of calculating \ac{eof} on IVT. 
...
The \acp{eof} were computed on the IWV, the zonal and meridional IVT fields respectively, but they also evaluated an approach of combining both IVT components in one data vector. 
...
It was shown by comparing the temporal patterns of the primary mode of \ac{eof} with the ENSO, that these patterns were related. 
...
In \cite{guirguis_circulation_2018}, the authors calculate rotated \ac{eof} on IVT data and try to analyze the relation between the 15 most dominant modes and the occurrence of atmospheric rivers (AR) on the USA west coast. 
...
While the main focus of \cite{yao_simulation_2013} is to evaluate and compare a regional air-sea coupled model, they also performed \ac{eof} analysis on the zonal and meridional components of IVT, respectively. 
...
\citeauthor{li_quasi-4-yr_2012} evaluated the connection of the IVT-\ac{eof} patterns to ENSO in the asian western northern Pacific.
They used a different approach then most in applying \ac{eof} to IVT, by concatenating the meridional and zonal components in one matrix and calculating \ac{eof} on it. 
...
2020 \cite{teale_patterns_2020}                 & SOMs                        & USA east                  & 1979 to 2017       & daily                    & all year                & IVT norm                       \\
2022  \cite{ayantobo_integrated_2022}                & \ac{eof}                         & China                     & 1979 to 2010       & daily                    & all year                & IVT norm                       \\
1982   \cite{salstein_modes_1983}               & \ac{eof}                         & Northern hemishpere       & 1958 to 1973       & monthly/yearly           & all year                & IWV, IVT\_u IVT\_v, combined   \\
2003  \cite{fernandez_analysis_2003}                & \ac{eof}                         & mediterranian sea         & 1948 to 1996       & 6hr                      & DJF                     & P                              \\
2005 \cite{zhou_atmospheric_2005}                 & \ac{eof}                         & China                     & 1951 to 1999       & monthly                  & JJA                     & P                              \\
2018 \cite{guirguis_circulation_2018}                 & \ac{eof}                         & USA (west coast)          & 1948 to 2017       & daily                    & NDJF                    & IVT norm (assumed)             \\
2015 \cite{kim_ensos_2015}                 & \ac{eof}                         & western USA               & 1979 to 2010       & 6hr                      & DJF                     & IVT norm (assumed)             \\
2018 \cite{zou_interdecadal_2018}                 & \ac{eof}                         & TEIOWP                    & 1961 to 2015       & monthly                  & JJA                     & IVT                            \\
2020 \cite{zou_investigating_2020}                 & \ac{eof}                         & TEIOWP                    & 1958 to 2018       & 6hr/monthly              & JJA                     & Integrated Water Vapor Sink    \\
2013 \cite{yao_simulation_2013}                 & \ac{eof}                         & East Asia                 & 1997 to 2002       &                          & JJA                     & IVT\_u IVT\_v                  \\
2012 \cite{li_quasi-4-yr_2012}                 & \ac{eof}                         & East Asia                 & 1979 to 2009       & monthly                  & summer                  & IVT                            \\
2018 \cite{wypych_atmospheric_2018}                 & \ac{eof}                         & Europe                    & 1981 to 2015       & daily                    & all year                & IWV, SLP                       
\end{tblr}
}
\end{table}
\todo{Check table again, fill gaps and improve Variable used for eof (maybe also add here compared vars, like SST and so on)}
...
Additionally, there has been no \ac{eof} IVT analysis for ensemble-scale data, just for reanalysis data (see Table \ref{tab:ivtpatterns-overview}) or actual weather station data.
...
Additionally, to my knowledge no future scenario pattern analysis has been performed with IVT, especially not with data from \ac{cmip} datasets.  
...
Unfortunately, none of these algorithms is available as a library \todo{double check if this is true}, which hinders application to a great extent. 
