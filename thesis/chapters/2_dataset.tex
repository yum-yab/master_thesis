...
Every seperate intial condition is a member of the simulation, and all members together are the whole ensemble.
...
Figure~\ref{fig:ssprcp} lists Tier 1, which are scenarios compareable to the old RCP scenarios. 
These scenarios are available in the MPI GE CMIP6, amongst some of Tier 2. 
...
The MPI GE CMIP6 consists of multiple coupled models: ECHAM6 for the atmosphere directly coupled to JSBACH for land and MPIOM for sea and sea-ice. 
...
So there were alternative functions tested for $h(p, P_S)$, but a final form for calculating pressure levels at any discrete sigma level $\tilde{\eta}$ is
