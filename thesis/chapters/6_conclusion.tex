...
%   \item Since it is the first time (to the authors knowledge) that \ac{ivt} and precipitation patterns are computed for whole Europe/Northern Atlantic, try to make understanding them easier by exploring their relationships to each other and to other variables to learn about their consequences. 
%   \item for the relationships the most important oscillations of europe (\ac{nao} and \ac{eap}) were chosen, which are established results of EOF analysis of sea level pressure data in europe. they are main drivers of winterly winds and weather
...
%
% \begin{itemize}
%   \item Analysis of stationary water vapor transport (monthly means) and their relationship with monthly precipitation and sea level pressure (represent dominant oscillations \ac{nao} and \ac{eap}) 
%   \item Results show two dominant modes of water vapor transport (quantified using \ac{ivt}), which seem quite stable across members and time
%   \item Precipitation EOFs reveal one quite stable mode and one questionable mode, the others seem degenerate
%   \item relationships between patterns was explored using correlation in two ways: one by comparing modes of PSL (= oscillations), \ac{ivt} and precipitation EOF and anoother by generating correlation map of a certain mode and data 
%   \item the former revealed quite strong correlations of the dominant modes of PSL and \ac{ivt} (and also precipitation). But they are also somewhat (weaker connected vice versa). 
%   \item Also a very strong relationship between the primary modes of \ac{ivt} and precipitation, which seems especially relevant for precipitation variability in the Iberian Peninsula. 
%   \item a bit weaker connection of both secondary EOF of \ac{ivt} and precipitation, the weaker connection could be due to the instability of the secondary precipitation EOF (which questions the usefullness of interpreting it) 
...
%   \item Also for the secondary mode of \ac{ivt} (and precipitation data): It is less influential on the west coast of europe. 
%   \item but a full evaluation and interpretation needs to be performed by a domain expert (meterologist or climate scientist)
% \end{itemize}


The analysis focused on stationary water vapor transport, represented by monthly means, and its relationship with monthly precipitation and sea level pressure, which reflect the dominant oscillations, \ac{nao} and \ac{eap}. 
...
% It is important to note that a comprehensive evaluation and interpretation of these findings should be conducted by a domain scientists, such as a meteorologist or climate scientists, to ensure accuracy and relevance.
